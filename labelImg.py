...
try:
    ...
    print(e)
    print("初次使用先运行`pyrcc5 -o resources.py resources.qrc; mv resources.py libs`")
    print("for first time use please issue `pyrcc5 -o resources.py resources.qrc; mv resources.py libs`")
    sys.exit(1)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None, txt_dir=None):
        ...
        print(txt_dir)
        if txt_dir is not None and not os.path.isdir(self.txt_dir):
            raise NotADirectoryError("txt_dir directory not exist")
        if self.txt_dir is None:
            self.user_set_txt_dir = False
        else:
            self.user_set_txt_dir = True
        ...
        # Create a widget for edit and diffc button
        self.tp_button = QCheckBox("true pos")
        self.tp_button.setChecked(False)
        self.tp_button.clicked.connect(self.add_tp)
        self.fp_button = QCheckBox("false pos")
        self.fp_button.setChecked(False)
        self.fp_button.clicked.connect(self.add_fp)
        self.fn_button = QCheckBox("false neg")
        self.fn_button.setChecked(False)
        self.fn_button.clicked.connect(self.add_fn)

        # Add some of widgets to list_layout
        list_layout.addWidget(self.edit_button)
        list_layout.addWidget(self.diffc_button)
        list_layout.addWidget(self.tp_button)
        list_layout.addWidget(self.fp_button)
        list_layout.addWidget(self.fn_button)
        ...
        add_true_pos = action("addtoTP", self.add_tp,
                              'j', 'verify', "shortcut: J")
        add_false_pos = action("addtoFP", self.add_fp,
                               'k', 'verify', "shortcut: K")
        add_false_neg = action("addtoFN", self.add_fn,
                               'l', 'verify', "shortcut: L")
        ...
        self.actions.beginner = (
            open, open_dir, change_save_dir, open_next_image, open_prev_image, verify, add_true_pos, add_false_pos,
            add_false_neg, save, save_format, None, create, copy, delete, None, zoom_in, zoom, zoom_out, fit_window,
            fit_width)
    ...
    def pic_in_txt(self, file_name):
        ...
        with open(file_name, "r") as f:
            ...
            for line in ls:
                line = line.strip('\n')  # 去掉列表中每一个元素的换行符
                if self.file_path == line:
                    flag = True
                    break
        print(self.file_path, flag)
        return flag, ls
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            if self.txt_dir is None:
                self.txt_dir = os.path.dirname(file_path)
            self.tp_button.setChecked(self.pic_in_txt(self.txt_dir + os.sep + "tp.txt")[0])
            self.fp_button.setChecked(self.pic_in_txt(self.txt_dir + os.sep + "fp.txt")[0])
            self.fn_button.setChecked(self.pic_in_txt(self.txt_dir + os.sep + "fn.txt")[0])
    ...
    def open_dir_dialog(self, _value=False, dir_path=None, silent=False):
        ...
        if not self.user_set_txt_dir:
            self.txt_dir = target_dir_path
    ...
    def add_tp(self):
        file_name = self.txt_dir + os.sep + "tp.txt"
        flag, ls = self.pic_in_txt(file_name)
        if not flag:
            print("add " + self.file_path + " to true pos")
            with open(file_name, "a+") as f:
                f.write(self.file_path + "\n")
        else:
            print("remove " + self.file_path + " to true pos")
            with open(file_name, "w+") as f:
                ls.remove(self.file_path + "\n")
                f.write("".join(ls))
        self.tp_button.setChecked(not flag)

    def add_fp(self):
        file_name = self.txt_dir + os.sep + "fp.txt"
        flag, ls = self.pic_in_txt(file_name)
        if not flag:
            print("add " + self.file_path + " to false pos")
            with open(file_name, "a+") as f:
                f.write(self.file_path + "\n")
        else:
            print("remove " + self.file_path + " to false pos")
            with open(file_name, "w+") as f:
                ls.remove(self.file_path + "\n")
                f.write("".join(ls))
        self.fp_button.setChecked(not flag)

    def add_fn(self):
        file_name = self.txt_dir + os.sep + "fn.txt"
        flag, ls = self.pic_in_txt(file_name)
        if not flag:
            print("add " + self.file_path + " to false neg")
            with open(file_name, "a+") as f:
                f.write(self.file_path + "\n")
        else:
            print("remove " + self.file_path + " to false neg")
            with open(file_name, "w+") as f:
                ls.remove(self.file_path + "\n")
                f.write("".join(ls))
        self.fn_button.setChecked(not flag)
...
def get_main_app(argv=None):
    ...
    argparser.add_argument('--txt', help='path to save tp/fp/fn txt')
    ...
    args.txt = args.txt and os.path.normpath(args.txt)

    # Usage : labelImg.py image classFile saveDir
    win = MainWindow(args.image_dir,
                     args.class_file,
                     args.save_dir, args.txt)
    win.show()
    return app, win
